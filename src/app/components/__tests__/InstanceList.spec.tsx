...
describe('InstanceList', () => {
  it('shows empty state when there are no instances', () => {
    render(<InstanceList instances={[]} onDelete={jest.fn()} onStop={jest.fn()} onStart={jest.fn()} onDeploy={jest.fn()} loading={false} />);
  ...
  it('renders instance rows', () => {
    render(<InstanceList instances={[instance]} onDelete={jest.fn()} onStop={jest.fn()} onStart={jest.fn()} onDeploy={jest.fn()} loading={false} />);
  ...
  it('shows dash when no route URL', () => {
    ...
    render(<InstanceList instances={[noRoute]} onDelete={jest.fn()} onStop={jest.fn()} onStart={jest.fn()} onDeploy={jest.fn()} loading={false} />);
    expect(screen.getByText('—')).toBeTruthy();
  });

  it('does not show empty state while loading', () => {
    const { container } = render(<InstanceList instances={[]} onDelete={jest.fn()} onStop={jest.fn()} onStart={jest.fn()} onDeploy={jest.fn()} loading={true} />);
